...
public class ColourableGraphImpl<T, V>
   ...
   * Advisable to set to false for performance reasons. If false, it is up to the caller to ensure
   * that the colour is acceptable beforehand.
   */
  public ColourableGraphImpl(GraphTopology<T> topology, ColourSpace<V> colourSpace,
  ...
  public void setColour(T nodeAddress, V colour) {
    boolean ok = (!checkConstraintOnSetColour) || (constraint == null) || constraint.check(this, nodeAddress, colour);
    if (ok) {
  ...
  public Collection<V> getPossibleColoursForNode(T node) {
    if (getColour(node) != null) {
      throw new IllegalArgumentException("The node " + node.toString() +
          " is already coloured. You should not be performing this operation.");
    }
    if (constraint == null) {
      return allColours;
    }
